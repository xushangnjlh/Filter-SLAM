...
class Particle:
    ...
    def H_Ql_jacobian_and_measurement_covariance_for_landmark(
        ...
        H = self.dh_dlandmark(self.pose,self.landmark_positions[landmark_number],
                              scanner_displacement)
        Ql = np.dot(np.dot(H,self.landmark_covariances[landmark_number]),H.T) + \
    ...
    def wl_likelihood_of_correspondence(self, measurement,
        ...
        delta_z = measurement - \
                  self.h_expected_measurement_for_landmark(landmark_number,scanner_displacement)
        Ql = self.H_Ql_jacobian_and_measurement_covariance_for_landmark(landmark_number,
                                                                        Qt_measurement_covariance,
                                                                        scanner_displacement)[1]
        wl = 1.0/(2*pi*sqrt(np.linalg.det(Ql))) * \
                exp(-0.5*(np.dot(np.dot(delta_z,np.linalg.inv(Ql)),delta_z)))
    ...
    def initialize_new_landmark(self, measurement_in_scanner_system,
        ...
        scanner_pose = (self.pose[0] + cos(self.pose[2]) * scanner_displacement,
                        ...
                        self.pose[2])
        measure_in_world_system = LegoLogfile.scanner_to_world(scanner_pose,
                                                               measurement_in_scanner_system)
        H = self.dh_dlandmark(self.pose,
                              measure_in_world_system,
                              scanner_displacement)
        sigma = np.dot(np.dot(np.linalg.inv(H),Qt_measurement_covariance),np.linalg.inv(H).T)
        self.landmark_positions.append(measure_in_world_system)  # Replace this.
    ...
    def update_landmark(self, landmark_number, measurement,
        ...
        Kalman_gain = np.dot(np.dot(self.landmark_covariances[landmark_number],H.T),np.linalg.inv(Ql))
        
        self.landmark_positions[landmark_number] += np.dot(Kalman_gain,Delta_z)
        self.landmark_covariances[landmark_number] = np.dot(np.eye(2)-np.dot(Kalman_gain,H),self.landmark_covariances[landmark_number])
    ...
    def decrement_visible_landmark_counters(self, scanner_displacement):
        ...
        for i in xrange(self.number_of_landmarks()):
            alpha = self.h_expected_measurement_for_landmark(i,scanner_displacement)[1]
            if Min <= alpha and alpha <= Max:
