# Comparison of the Kalman filter (continuous Gaussian distribution) and the histogram filter.
...
def kalman_filter_step(belief, control, measurement):
    ...
    correction = Density(prediction.mu + Kalman_gain * (measurement.mu - prediction.mu),
                         (1 - Kalman_gain) * prediction.sigma2);
    return (prediction, correction)
