...
def find_cylinder_pairs(cylinders, reference_cylinders, max_radius):
    ...
    for i in xrange(len(cylinders)):
        for j in xrange(len(reference_cylinders)):
            if (cylinders[i][0] - reference_cylinders[j][0])**2+(cylinders[i][1] - reference_cylinders[j][1])**2<= max_radius**2:
...
if __name__ == '__main__':
    ...
    # Also read the reference cylinders from robot_arena_landmarks.txt
    logfile.read("robot_arena_landmarks.txt")
    reference_cylinders = [l[1:3] for l in logfile.landmarks]
    print [l[0:3] for l in logfile.landmarks]
