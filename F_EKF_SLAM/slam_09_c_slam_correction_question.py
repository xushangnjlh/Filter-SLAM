...
class ExtendedKalmanFilterSLAM:
    ...
    def predict(self, control):
        ...
        V3 = self.dg_dcontrol(self.state[0:3], control, self.robot_width)
        N = self.number_of_landmarks
        G = zeros((2*N+3,2*N+3))
        G[0:3,0:3] = G3
        G[3:2*N+3,0:3] = zeros((2*N,3))
        G[0:3,3:2*N+3] = zeros((3,2*N))
        G[3:2*N+3,3:2*N+3] = eye(2*N)
        
        V = zeros((2*N+3,2))
        V[0:3,0:2] = V3
        R = dot(V, dot(control_covariance, V.T))
        self.covariance = dot(G, dot(self.covariance, G.T)) + R
        self.state = concatenate((self.g(self.state[0:3], control, self.robot_width),self.state[3:self.number_of_landmarks*2+3]))
    ...
    def add_landmark_to_state(self, initial_coords):
        ...
        self.state = concatenate((self.state,[initial_coords[0],initial_coords[1]]))
        dim = self.covariance.shape[0]
        M = zeros((dim+2,dim+2))
        M[0:dim,0:dim] = self.covariance
        M[dim:dim+2,0:dim] = zeros((2,dim))
        M[0:dim,dim:dim+2] = zeros((dim,2))
        M[dim:dim+2,dim:dim+2] = diag([10**5,10**5])
        self.covariance = M
    ...
    def correct(self, measurement, landmark_index):
        ...
        # first initialize the shape of H (2*(3+self.number_of_landmarks))
        H = concatenate((H3,zeros((2,2*self.number_of_landmarks))),axis=1)
        H[:,3+landmark_index*2:5+landmark_index*2:] = - H3[:,0:2]
